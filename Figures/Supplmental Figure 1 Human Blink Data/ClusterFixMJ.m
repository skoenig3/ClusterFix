function [fixationstats] = ClusterFixMJ(eyedat,samprate)
...
x = eyedat(1,:);%*24+400; %converts dva to pixel and data from [-400,400] to [0,800]
y = eyedat(2,:);%*24+300; %converts dva to pixel and from [-300,300] to [0,600]
...
vel = vel(1:end-samprate*1000);
rot = zeros(1,length(xss)-2*samprate*1000); %angular velocity
dist = zeros(1,length(xss)-2*samprate*1000); %distance
for a = 1:length(xss)-3;
...
pointsac = NaN(length(saccadetimes),6);
...
recalc_meanvalues(1,:) = mean(pointfix);
recalc_meanvalues(2,:) = mean(pointsac);
recalc_stdvalues(1,:) = nanstd(pointfix);
recalc_stdvalues(2,:) = nanstd(pointsac);

fixationstats.fixationtimes = fixationtimes;
fixationstats.fixations = fixations;
fixationstats.saccadetimes = saccadetimes;
fixationstats.FixationClusterValues = pointfix;
fixationstats.SaaccadeClusterValues = pointsac;
fixationstats.MeanClusterValues = recalc_meanvalues;
fixationstats.STDClusterValues = recalc_stdvalues;
fixationstats.XY = [x;y];
fixationstats.variables = variables;
end
...
%Iter vs Intra distance points by cluster-mod of SILHOUETTE (just
