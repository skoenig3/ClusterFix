...
% fsigm = @(param,xval) param(1)+(param(2)-param(1))./(1+e.^((param(3)-xval)*param(4)))
...
%p(1)=min; p(2)=max-min; p(3)=x50; p(4)=slope como em Y=Bottom + (Top-Bottom)/(1+e^((LogEC50-X)*HillSlope))
%f = @(p,x) p(1) + (p(2)-p(1)) ./ (1 + e.^((p(3)-x)*p(4)));
