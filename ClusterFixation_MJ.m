function [fixationstats] = ClusterFixation_MJ(eyedat,samprate)
...
variables = {'Dist','Vel','Accel','Rotation'}; %parameters to be extracted
...
for cndlop = 1:length(eyedat)
    ...
    x = eyedat{cndlop}(1,:);%*24+400; %converted data from [-400,400] to [0,800]
    y = eyedat{cndlop}(2,:);%*24+300; %converted data from [-300,300] to [0,600]
...
%     saccadetimes = saccadetimes + 200;
    ...
    pointfix = NaN(length(fixationtimes),6);
